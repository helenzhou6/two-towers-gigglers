...
def collate_for_inference(batch_of_lists):
    ...
    batch_of_tensors = [torch.tensor(item, dtype=torch.long) for item in batch_of_lists]
    
    # Concatenate all tensors in the batch
    flat_tensor = torch.cat(batch_of_tensors, dim=0)

    # Create the offsets tensor
    lengths = torch.tensor([len(t) for t in batch_of_tensors], dtype=torch.long)
...
def evaluate(config):
    ...
    if config.get("wandb_log", False):
        ...
        wandb.init(project="two-towers-gigglers", entity="mlx-gg", anonymous="allow")
    ...
    queries_path = load_artifact_path('query_processed_validation', file_extension='parquet')
    queries_df = pd.read_parquet(queries_path)

    # Load all unique documents from the validation set
    docs_path = load_artifact_path('docs_processed_validation', file_extension='parquet')
    ...
    query_to_positives = queries_df.groupby('query').agg(lambda x: list(x))['doc'].to_dict()
    ...
    with torch.no_grad():
        for (doc_flat, doc_off) in tqdm(doc_dataloader, desc="Embedding Documents"):
            ...
            doc_embeddings.append(embeddings.cpu()) # Move to CPU to save GPU memory
    ...
    with torch.no_grad():
        for i, (q_flat, q_off) in enumerate(tqdm(query_dataloader, desc="Evaluating Queries")):
            ...
            q_vecs = query_model((q_flat, q_off)).cpu() # Shape: [batch_size, emb_dim]

            # Calculate cosine similarity between each query and all docs
            similarity_scores = F.cosine_similarity(q_vecs.unsqueeze(1), doc_index.unsqueeze(0), dim=2)
            
            # For each query in the batch, calculate its AP
            for j in range(similarity_scores.shape[0]):
                query_idx_in_full_list = i * BATCH_SIZE + j
                current_query_tokens = tuple(query_list[query_idx_in_full_list])
                
                # Create the binary relevance label vector for this query
                # Convert lists to tuples for correct 'in' comparison
                positive_docs_for_query = {tuple(item) for item in query_to_positives[current_query_tokens]}
                
                # is_relevant is 1 if doc is in positives, 0 otherwise
                is_relevant = np.array([1 if tuple(doc) in positive_docs_for_query else 0 for doc in doc_list])
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate a Two-Towers model.")
    parser.add_argument(
...
if __name__ == "__main__":
    main()
    # evaluate({
    #     'query_model_artifact': 'query_model:latest',
    #     'doc_model_artifact': 'doc_model:latest',
    #     'wandb_log': True
    # }) 